...
import React, { useState, useEffect, useRef } from 'react';
import Webcam from 'webcam-easy';  // Webcam control library
import jsQR from 'jsqr';  // QR code scanning library
import axios from 'axios';  // For API calls
import styles from './ScanQR.module.css';

const ScanQRPage = () => {
    const [cameraActive, setCameraActive] = useState(false); // Camera active state
    const [popupActive, setPopupActive] = useState(false); // Show/Hide popup state
    const [webcam, setWebcam] = useState(null); // Store webcam instance
    ...
    const startCamera = () => {
        if (!webcam) {
            const webcamInstance = new Webcam(
                videoRef.current,
                'user',
                canvasRef.current  // Canvas that will be used for processing
            );
            webcamInstance.start()
                .then(() => {
                    setCameraActive(true);
    ...
    const scanQRCode = () => {
        ...
        if (canvas && video && cameraActive && webcam) {
            ...
            if (context) {
                ...
                if (qrCode) {
                    ...
                    fetchLocationData(qrCode.data);  // Fetch location data based on QR code content
                }
            }
        }
    };

    // Fetch location data from the backend based on QR code data
    const fetchLocationData = (locationId) => {
        axios.get(`http://localhost:5000/api/location/${locationId}`)
            .then(response => {
        ...
        <div className={styles.mainContainer}>
            <div className={styles.Scan}>
                ...
                <div className={styles.cameraMask}>
                    ...
                    {/* Canvas used for scanning the QR code */}
                    <canvas
                        ref={canvasRef}
                        width={640}
                        height={480}
                        style={{ display: 'none' }}  // Hide the canvas, it’s used for QR code scanning, not display
                    ></canvas>
                </div>
